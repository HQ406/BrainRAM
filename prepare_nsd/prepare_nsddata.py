...
stim_order_f = NSD_PATH + '/nsd/nsddata/experiments/nsd/nsd_expdesign.mat'
...
roi_dir = NSD_PATH + '/nsd/nsddata/ppdata/subj{:02d}/func1pt8mm/roi/'.format(sub)
betas_dir = NSD_PATH + '/nsd/nsddata_betas/ppdata/subj{:02d}/func1pt8mm/betas_fithrf_GLMdenoise_RR/'.format(sub)
...
f_stim = h5py.File(NSD_PATH + '/nsd/nsddata_stimuli/stimuli/nsd/nsd_stimuli.hdf5', 'r')
...
nsda = NSDAccess(NSD_PATH + '/nsd/')


cocoid_array = np.empty(num_test, dtype=np.int32)
captions_array = np.empty((num_test,5),dtype=np.dtype('<U250'))
for i,idx in enumerate(test_im_idx):
    print(f"{i+1} / {num_test}")
    cocoid_array[i] = nsda.read_image_coco_info([idx])[0]['image_id']
    captions_array[i,:] = [x['caption'] for x in nsda.read_image_coco_info([idx])][:5]
    # cocoid_array[i] = idx
np.save('processed_data/subj{:02d}/nsd_test_cap_sub{}.npy'.format(sub,sub),captions_array )
np.save('processed_data/subj{:02d}/nsd_test_cocoid_sub{}.npy'.format(sub,sub),cocoid_array)

cocoid_array = np.empty(num_train, dtype=np.int32)
captions_array = np.empty((num_train,5),dtype=np.dtype('<U250'))
for i,idx in enumerate(train_im_idx):
    print(f"{i+1} / {num_train}")
    cocoid_array[i] = nsda.read_image_coco_info([idx])[0]['image_id']
    captions_array[i,:] = [x['caption'] for x in nsda.read_image_coco_info([idx])][:5]
np.save('processed_data/subj{:02d}/nsd_train_cap_sub{}.npy'.format(sub,sub),captions_array )
np.save('processed_data/subj{:02d}/nsd_train_cocoid_sub{}.npy'.format(sub,sub),cocoid_array)
